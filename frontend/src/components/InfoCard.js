...
export default function InfoCard({
  ...
  return (
    <Card className={`mb-6 h-auto ${className} bg-blue-50 rounded-lg xs:rounded-2xl`}>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-2 xs:gap-4 w-full">
          ...
          <div className="flex items-start gap-1 sm:gap-3 md:gap-4">
            {isCheckIcon ? (
              ...
              <Image src="/infoIcon.png" width={20} height={20} alt="Info Icon" className="w-4 md:w-5 h-4 md:h-5 hidden sm:block" />
            )}
            <div>
          ...
          {buttonText && (
            <div className="flex items-center shrink-0 self-auto w-full sm:w-auto">
              ...
              >
                ...
                {ButtonIcon && <ButtonIcon className={`w-4 h-4 ${buttonIconClassName}`} />}
                {buttonText}
              </Button>
            </div>
          )}
